#include <KeyExtractorIncludes.h>
#include <iostream>
...
std::string KeyExtractor::parseJsonForKey(const std::string& jsonData) {
	boost::json::value json = boost::json::parse(jsonData);
	std::string encryptedKey = boost::json::value_to<std::string>(json.at("os_crypt").at("encrypted_key"));
	return encryptedKey;
}
